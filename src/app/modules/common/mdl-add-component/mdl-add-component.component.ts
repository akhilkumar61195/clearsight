...
import { MdlBuilderService } from '../../schematic/assembly-builder/builder/mdl-builder.service';
import { PRIME_IMPORTS } from '../../../shared/prime-imports';

@Component({
  selector: 'app-mdl-add-component',
  standalone: true,
  imports: [...PRIME_IMPORTS],
  templateUrl: './mdl-add-component.component.html',
  styleUrl: './mdl-add-component.component.scss'
})
export class MdlAddComponentComponent implements OnInit {
  ...
  selectedRowData: any;
  // mdlRecords: WritableSignal<Array<masterdatalibraryModel>> = this.mdlBuilderService.mdlRecords; // Signal to hold MDL records
  mdlRecords = this.mdlBuilderService.filteredMdlRecords; // Signal to hold MDL records
  ...
  mdlColumnDefs = [
    ...
    minWidth: 220,
    filterParams: {
      // optional: keep values sorted
      suppressMiniFilter: false,
      defaultToNothingSelected: true,
    }
  },
    { headerName: 'Material Description', field: 'materialDescription', sortable: true, filter: true, minWidth: 370 },
    ...
    // added material group column
    { headerName: 'Material Group', field: 'materialGroup', sortable: true, filter: true, minWidth: 150 },
    { headerName: 'UoM (FT or EA)', field: 'uom', sortable: true, filter: true, minWidth: 170 }, // <--- New column Unit Of Measurement
    { headerName: 'Manufacturer SAP #', field: 'vendorSapnumber', sortable: true, filter: true, minWidth: 170 },// <--- New column Vendor SAP Number
    // { headerName: 'Supplier', field: 'organizationName', sortable: true, filter: true },
    { headerName: 'Legacy Reference #', field: 'legacyRefNumber', sortable: true, filter: true, minWidth: 140 },
    {
    ...
    { headerName: 'Axial Strength (PSI)', field: 'axialStrength', sortable: true, filter: true, minWidth: 220 },
  ...
  ngOnInit() {
    this.loadMasterData();
    // console.log('266',this.moduleName);
  }
  ...
  onGridReady(params: any) {
    ...
    // added ag-grid filter to select only Group 1 and blank values by default
    this.gridApi.getColumnFilterInstance('groupName')
    .then(filter => {
      if (filter) {
        (filter as any).setModel({
          values: ['', 'Group 1']
        });
  ...
  /**
   * Load master data from the MDL signal service and update the grid.
   * This method fetches all records and updates the filteredMaterials and totalRecords properties.
   */
  loadMasterData() {
  ...
  addRecord() {
    ...
    if (this.moduleName == 'schematicAddComponent') {
      const selectedIds = selectedRows.map(row => ({
        groupName:row.groupName, // added groupname
      ...
      if (selectedIds.length > 0) {
        ...
        assemblyDataArray = selectedIds.map(data => {
          ...
          newAssembly.groupName = data.groupName;
          return newAssembly;
        });
    ...
    if (this.moduleName == 'thorAddComponent') {
      const selectedIds = selectedRows.map(row => ({
        materialId: row.materialNumber,
        materialShortDesc: row.materialDescription,
        componentTypeName: row.componentTypeName,
      }));
     
      this.addSelected.emit(selectedIds);
    }
  }
