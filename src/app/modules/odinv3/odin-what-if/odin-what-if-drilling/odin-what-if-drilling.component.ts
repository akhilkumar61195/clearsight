...
@Component({
  selector: 'app-odin-what-if-drilling',
  standalone:true,
      imports:[...PRIME_IMPORTS,
        ConfirmationDialogComponent
      ],
  templateUrl: './odin-what-if-drilling.component.html',
  styleUrl: './odin-what-if-drilling.component.scss',
    encapsulation: ViewEncapsulation.None,
})
export class OdinWhatIfDrillingComponent implements AfterViewInit, OnDestroy{
displayConfirmationComponentDialog:boolean=false;
  ...
  private whatIfDrillingSubscription: Subscription = new Subscription();
  ...
  ngOnDestroy(): void {
    this.whatIfDrillingSubscription?.unsubscribe();
  }
  ...
  ngAfterViewInit(): void {
    ...
    getWhatIfList() {
      ...
      this.loading = true;
      this.whatIfDrillingSubscription = this.odinV2Service.getOdinWhatIfDataSearch(body, params,1,this.selectedView)
        .subscribe({
          next: (response: any) => {
            this.loading = false;
    ...
    onClickSave(isFromRunWhatIfAnalysis?: boolean) {
      ...
      this.whatIfDrillingSubscription = this.odinV2Service.editOdinWhatIfWells(this.tableDataEdited,1,this.selectedView)
        .subscribe({
          next: (response: any) => {
    ...
    reset() {
      ...
      this.tableDataEdited = [];
      this.unsavedChanges = false;
      this.whatIfDrillingSubscription = this.odinV2Service.resetOdinWhatIfWells(1,this.selectedView)
        .subscribe({
          next: (response: any) => {
            if (response && response.success) {
