...
import { AllApplications } from '../../../common/enum/common-enum';
...
@Component({
  ...
  standalone: true,
  imports: [AdminTabComponent,...PRIME_IMPORTS,DeleteConfirmationDialogComponent,
    ListEditorComponentLayoutComponent
  ],
  templateUrl: './admin-list-editor.component.html',
  styleUrls: ['./admin-list-editor.component.scss'],
})
export class AdminListEditorComponent implements OnInit, OnDestroy {
  ...
  tyrOptionsButtons: DropDownOptions[];
  ...
  /** Name of item to be displayed in add confirmation dialog */
  addItemDataName: string;
  /** Controls visibility of delete confirmation dialog */
  displayDeleteComponentDialog: boolean = false;
  /** Controls visibility of add confirmation dialog */
  displayAddComponentDialog: boolean = false;
  ...
  // List editors dropdowns
  selectedApplication: string = 'ADMIN'; // default selection of Admin
  applicationList: Applications[] = [];
  businessUnits: BusinessUnit[] = [];
  applications: Applications[] = [];
  selectedBU: string = 'Gulf of America';
  allBu: BusinessUnit[] = [];
  ...
  storeListEditorData: WritableSignal<listBuilder[]> = this.builderService.listEditor;

  // Tyr options
  showTyrOptions: boolean = false;
  tyrSelectedView: number;
  tyrViewOptionsButtons: DropDownOptions[];
  ...
  columnDefsComponentType = this.createColumnDefs(
    'Component Type',
    'componentTypeName',
    'componentType'
  );
  columnDefsConnectionConfig = this.createColumnDefs(
    'Connection Config',
    'connectionConfigName',
    'connectionConfig'
  );
  columnDefsEndConnections = this.createColumnDefs(
    'End Connections',
    'endConnectionName',
    'endConnection'
  );
  columnDefsMaterialGrade = this.createColumnDefs(
    'Material Grade',
    'materialGrade',
    'materialGrade'
  );
  columnDefsRange = this.createColumnDefs('Range', 'rangeName', 'range');
  columnDefsSupplier = this.createColumnDefs(
    'Supplier',
    'organizationName',
    'supplier'
  ); // Replacing organization with supplier
  columnDefsManufacturer = this.createColumnDefs(
    'Manufacturer',
    'organizationName',
    'manufacturerType'
  );
  columnDefsConnectionType = this.createColumnDefs(
    'Connection Type',
    'name',
    'connectionType'
  );
  columnDefsAssemblyType = this.createColumnDefs(
    'Assembly Type',
    'assemblyTypeName',
    'assemblyType'
  );
  ...
  showFunctionsToggle: boolean = false; // Flag for displaying completion / drilling toggle
  ...
  constructor(
    ...
    private builderService: ListEditorBuilderService,
    ...
    this.builderService.selectedListOption.set(-1);
    effect(() => {
      this.listEditorData = this.storeListEditorData();
      this.tyrViewOptionsButtons = this.builderService.listEditorOptionsButtons();
    });
  }

  ngOnDestroy(): void {
    // Resetting the signals
    this.builderService.selectedApplicationId.set(null);
    // this.builderService.selectedBuId.set(-1); // Commenting this for now, as we decided to keep the BUID as 1 for all the cases
    this.builderService.listEditor.set(null);
    this.builderService.listEditorOptionsButtons.set([]);
    this.builderService.selectedFunctionId.set(-1);
    this.builderService.selectedView.set(1);
  }

  // method called on complete event triggered for autocomplete app selector dropdown
  filterCoordinators(event: any) {
    const query = event.query.toLowerCase();
    this.applications = this.applicationList.filter((item) =>
      item.applicationName.toLowerCase().includes(query)
    );
  }

  // method called on complete event triggered for autocomplete BU selector dropdown
  filterBu(event: any) {
    const query = event.query.toLowerCase();
    this.allBu = this.businessUnits.filter((item) =>
      item.buName.toLowerCase().includes(query)
    );
  }
  ...
  ngOnInit() {
    ...
    // By default, the selection willl be admin, so setting function Id as 1
    this.builderService.selectedFunctionId.set(1);
  ...
  getUserDetails() {
    ...
    // Checking access for editability for drilling and completions list editor
    this.isEditableField = this.authService.isFieldEditable(
      'isAdminAddListEditor'
    );
    this.isDeleteEditable = this.authService.isFieldEditable(
  ...
  onTyrViewChange(selectedValue: number): void {
    this.builderService.selectedListOption.set(selectedValue);
    this.builderService.getConfiguartions();
  }

  // Get the list of applications
  getApplications() {
    // Changing the getApplication endpoint to new where all the modules having list editor will be shown
    this.lookupsService.getListEditorApplications().subscribe({
      next: (response) => {
  ...
  getAllBusinessUnits() {
    this.configurationValuesService.getBusineedUnitList().subscribe({
      next: (response) => {
  ...
  // Sets the app id after selection
  setApplicationId(event) {
    this.isBuSelected = false;
    this.showTyrOptions = false;
    this.builderService.selectedListOption.set(-1);
    const appName = (event?.value.applicationName).toLowerCase();
    this.builderService.selectedApplicationId.set(event?.value.id);
    // this.builderService.selectedBuId.set(-1);  // Commenting this for now, as we decided to keep the BUID as 1 for all the cases
    if (appName === AllApplications.system.toLowerCase()) {
      this.isBuSelected = true;
      // this.builderService.selectedBuId.set(1); // Commenting this for now, as we decided to keep the BUID as 1 for all the cases
      this.builderService.selectedFunctionId.set(1); // Setting functionId as 1
      this.selectedBU !== null ? this.setDefaultBU() : '';
      this.showFunctionsToggle = false;
    } else if (appName === AllApplications.tyr.toLowerCase()) {
      // Handling cases for tyr options dropdown
      this.showFunctionsToggle = false;
      this.showTyrOptions = true;
      this.builderService.selectedListOption.set(1);
      this.builderService.getListEditorOptions();
      this.builderService.getConfiguartions();
      this.listEditorData = this.builderService.listEditor();
      this.tyrSelectedView = this.builderService.selectedView();
    } else {
      // Fix for admin dropdown
      if (
        appName === AllApplications.admin.toLowerCase() ||
        appName === AllApplications.schematic.toLowerCase()
      ) {
        this.showFunctionsToggle = false;
        this.builderService.getConfiguartions(-1);
        this.listEditorData = this.builderService.listEditor();
      } else {
        this.showFunctionsToggle = true;
        this.builderService.selectedFunctionId.set(2); // Setting functionId as 2 for completions
        this.builderService.getConfiguartions();
        this.listEditorData = this.builderService.listEditor();
      }
    }
  ...
  // Sets the BU id after selection
  setBuId(event) {
    // this.builderService.selectedBuId.set(event?.value.appid); // Commenting this for now, as we decided to keep the BUID as 1 for all the cases
    this.setDefaultBU();
  }
   ...
   */
  getComponentType() {
    this.inventoryService.getComponentType().subscribe(
      (response) => {
        this.componentTypes = response.filter(
          (item: any) => item.isDeleted != 1
        );
      },
      (error) => {
        console.error('Error fetching record', error);
  ...
  get filteredComponentTypes() {
    if (
      !this.componentTypeText ||
      this.componentTypeText.trim() === '' ||
      this.componentTypeText === '0'
    ) {
    ...
    return this.componentTypes.filter((item) =>
      item.componentTypeName
        .toLowerCase()
        .includes(this.componentTypeText.toLowerCase())
    );
  }
  ...
  addComponentType() {
    ...
    this.inventoryService.addComponentType(componentType).subscribe(
      (response) => {
        this.getComponentType();
        this.componentTypeText = '';
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Component Type saved successfully',
        });
      },
      (error) => {
   ...
   */
  getConnectionConfig() {
    this.inventoryService.getConnectionConfig().subscribe(
      (response) => {
        this.connectionConfigs = response.filter(
          (item: any) => item.isDeleted != 1
        );
      },
      (error) => {
  ...
  get filteredConnectionConfigs() {
    if (
      !this.connectionConfigText ||
      this.connectionConfigText.trim() === '' ||
      this.connectionConfigText === '0'
    ) {
      return this.connectionConfigs;
    }
    return this.connectionConfigs.filter((item) =>
      item.connectionConfigName
        .toLowerCase()
        .includes(this.connectionConfigText.toLowerCase())
    );
  }
  ...
  addConnectionConfig() {
    ...
    this.inventoryService.addConnectionConfig(connectionConfig).subscribe(
      (response) => {
        this.getConnectionConfig();
        this.connectionConfigText = '';
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Connection Config saved successfully',
        });
      },
      (error) => {
        console.error('Error saving record', error);
   ...
   */
  getEndConnections() {
    this.inventoryService.getEndConnections().subscribe(
      (response) => {
        this.endConnections = response.filter(
          (item: any) => item.isDeleted != 1
        );
      },
      (error) => {
  ...
  get filteredEndConnections() {
    if (
      !this.endConnectionsText ||
      this.endConnectionsText.trim() === '' ||
      this.endConnectionsText === '0'
    ) {
      return this.endConnections;
    }
    return this.endConnections.filter((item) =>
      item.endConnectionName
        .toLowerCase()
        .includes(this.endConnectionsText.toLowerCase())
    );
  }
  ...
  addEndConnection() {
    ...
    this.inventoryService.addEndConnection(endConnection).subscribe(
      (response) => {
        this.getEndConnections();
        this.endConnectionsText = '';
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'End Connection saved successfully',
        });
      },
      (error) => {
        console.error('Error saving record', error);
   ...
   */
  getMaterialGrade() {
    this.inventoryService.getMaterialGrade().subscribe(
      (response) => {
        this.materialGrades = response.filter(
          (item: any) => item.isDeleted != 1
        );
      },
      (error) => {
  ...
  get filteredMaterialGrades() {
    if (
      !this.materialGradeText ||
      this.materialGradeText.trim() === '' ||
      this.materialGradeText === '0'
    ) {
    ...
    return this.materialGrades.filter((item) =>
      item.materialGrade
        .toLowerCase()
        .includes(this.materialGradeText.toLowerCase())
    );
  }
  ...
  addMaterial() {
    ...
    this.inventoryService.addMaterialGrade(materialGrade).subscribe(
      (response) => {
        this.getMaterialGrade();
        this.materialGradeText = '';
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Material Grade saved successfully',
        });
      },
      (error) => {
        console.error('Error saving record', error);
   ...
   */
  getRange() {
    this.inventoryService.getRange().subscribe(
      (response) => {
  ...
  addRange() {
    ...
    range.isDeleted = 0;
    this.inventoryService.addRange(range).subscribe(
      (response) => {
        this.getRange();
        this.rangeText = '';
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Range saved successfully',
        });
      },
      (error) => {
        console.error('Error saving record', error);
  ...
  get filteredSuppliers() {
    ...
    return this.suppliers.filter((item) =>
      item.organizationName
        .toLowerCase()
        .includes(this.supplierText.toLowerCase())
    );
  }
  ...
  addSupplier() {
    ...
    this.supplierService.addSupplier(organization).subscribe(
      (response) => {
        this.getSupplier();
        this.supplierText = '';
        this.messageService.add({
   ...
   */
  getConnectionType() {
    this.connectionTypeService.getConnectionType().subscribe(
      (response) => {
        this.connectionTypes = response.filter(
          (item: any) => item.isDeleted != 1
        );
      },
      (error) => {
  ...
  get filteredConnectionTypes() {
    if (
      !this.connectionTypeText ||
      this.connectionTypeText.trim() === '' ||
      this.connectionTypeText === '0'
    ) {
  ...
  addConnectionType() {
    ...
    this.connectionTypeService.addConnectionType(connectionType).subscribe(
      (response) => {
        this.getConnectionType();
        this.connectionTypeText = '';
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Connection Type saved successfully',
        });
      },
      (error) => {
        console.error('Error saving record', error);
   ...
   */
  getAssemblyType() {
    this.assemblyTypeService.getAssemblyType().subscribe(
      (response) => {
        this.assemblyTypes = response.filter(
          (item: any) => item.isDeleted != 1
        );
      },
      (error) => {
  ...
  get filteredAssemblyTypes() {
    if (
      !this.assemblyTypeText ||
      this.assemblyTypeText.trim() === '' ||
      this.assemblyTypeText === '0'
    ) {
    ...
    return this.assemblyTypes.filter((item) =>
      item.assemblyTypeName
        .toLowerCase()
        .includes(this.assemblyTypeText.toLowerCase())
    );
  }
  ...
  addAssemblyType() {
    ...
    this.assemblyTypeService.addAssemblyType(assemblyType).subscribe(
      (response) => {
        this.getAssemblyType();
        this.assemblyTypeText = '';
        this.messageService.add({
          severity: 'success',
          summary: 'Success',
          detail: 'Assembly Type saved successfully',
        });
      },
      (error) => {
        console.error('Error saving record', error);
   ...
   */
  getManufacturerType() {
    this.manufacturerService.getManufacturerType().subscribe(
      (response) => {
        this.manufacturerType = response.filter(
          (item: any) => item.isDeleted != 1
        );
      },
      (error) => {
  ...
  get filteredManufacturer() {
    if (
      !this.manufacturerText ||
      this.manufacturerText.trim() === '' ||
      this.manufacturerText === '0'
    ) {
    ...
    return this.manufacturerType.filter((item) =>
      item.organizationName
        .toLowerCase()
        .includes(this.manufacturerText.toLowerCase())
    );
  }
  ...
  addManufacturerType() {
    ...
    this.manufacturerService.addManufacturerType(manufacturerType).subscribe(
      (response) => {
        this.getManufacturerType();
        this.manufacturerText = '';
        this.messageService.add({
  ...
  onViewChange(selectedValue: number): void {
      ...
      this.builderService.selectedFunctionId.set(selectedValue);
      this.setDefaultBU();
  }
  ...
  addRbw() {
    ...
    this.rbwService.addRbw(rbw).subscribe(
      (response) => {
        this.getRbw();
        this.rbwText = 0;
        this.messageService.add({
  ...
  private addOrUpdateEdit(edit: { type: string; data: any }) {
    ...
    const existingEditIndex = this.editedRows.findIndex(
      (e) =>
        e.type === edit.type &&
        this.getIdForType(e.type, e.data) ===
          this.getIdForType(edit.type, edit.data)
    );

    if (existingEditIndex !== -1) {
      // Update the existing edit with the new value
      this.editedRows[existingEditIndex].data.originalValue =
        this.editedRows[existingEditIndex].data.originalValue;
      this.editedRows[existingEditIndex].data[
        this.getFieldNameForType(edit.type)
      ] = edit.data[this.getFieldNameForType(edit.type)];
    } else {
      // Add as a new edit
    ...
    const updatePromises = Object.values(latestEdits).map((edit: any) => {
      ...
      switch (edit.type) {
        ...
        case 'endConnection':
          ...
          return this.inventoryService
            .updateEndConnection(endConnection)
            .toPromise();
        ...
        case 'materialGrade':
          ...
          return this.inventoryService
            .updateMaterialGrade(materialGrade)
            .toPromise();
        ...
        case 'connectionType':
          ...
          return this.connectionTypeService
  ...
  deleteAssemblyDialog(rowData: any, type: string) {
    ...
    this.deleteItemDataName = rowData
      .map((element) => element[tableType])
      .join(', ');
  ...
  deleteConfirm() {
    ...
    let dataTypeID: string = this.getDeletedItemsTypes(
      this.deleteItemType,
      'value'
    );
    ...
    if (this.deleteItemType === 'manufacturerType') {
      ...
      const deleteObservables = this.deleteItemData.map((item) =>
        this.manufacturerService
          .deleteManufacturerType(item[dataTypeID])
          .toPromise()
      );
    ...
    let deletePromise: Promise<any> = this.listEditorService
      .deleteMultipleListEditorItems(
        this.deleteItemType,
        this.deleteItemData.map((ele) => ele[dataTypeID])
      )
      .toPromise();
    if (deletePromise) {
      deletePromise
  ...
  sendSelectedRows(gridType: string) {
    const nameIndex = this.tableDeletingData.findIndex(
      (element) => element.type === gridType
    );
    const selectedRows =
      this.tableDeletingData[nameIndex]?.gridApi.getSelectedRows();
    this.deleteAssemblyDialog(selectedRows, gridType);
  }

  //Deletes selected rows from the grid */
  openAddSelectionPopUp(searchText: string, selection: string) {
    this.addItemDataName = searchText;
    this.displayAddComponentDialog = true;
    this.selectedColumn = selection;
  }

  // Call the function on click of save

  confirmSave() {
    this.displayAddComponentDialog = false;
    switch (this.selectedColumn) {
      case 'componentType':
        return this.addComponentType();
      case 'connectionConfig':
        return this.addConnectionConfig();
      case 'endConnection':
        return this.addEndConnection();
      case 'materialGrade':
      ...
      case 'manufacturerType':
        return this.addManufacturerType();
      case 'assemblyType':
        return this.addAssemblyType();
      case 'rbw':
        return this.addRbw();
      default:
        return null;
  ...
  getDeletedItemsTypes(dataType: string, returnType?: string): string {
    ...
    switch (dataType) {
      case 'componentType':
        dataTypeID =
          returnType === 'value' ? 'componentTypeId' : 'componentTypeName';
        break;
      case 'connectionConfig':
        dataTypeID =
          returnType === 'value'
      ...
      case 'endConnection':
        dataTypeID =
          returnType === 'value' ? 'endConnectionId' : 'endConnectionName';
        break;
      case 'materialGrade':
        dataTypeID =
          returnType === 'value' ? 'materialGradeId' : 'materialGrade';
        break;
      case 'range':
      ...
      case 'supplier': // Replacing organization with supplier
        dataTypeID =
          returnType === 'value' ? 'organizationId' : 'organizationName';
        break;
      case 'manufacturerType':
        dataTypeID =
          returnType === 'value' ? 'organizationId' : 'organizationName';
      ...
      case 'assemblyType':
        dataTypeID =
          returnType === 'value' ? 'assemblyTypeId' : 'assemblyTypeName';
        break;
      case 'rbw':
